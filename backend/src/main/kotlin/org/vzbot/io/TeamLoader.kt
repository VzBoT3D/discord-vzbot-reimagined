...
object TeamLoader {
    ...
    private fun loadTeamMembers() {
        val teamRole = KtBot.getRole(env[EnvVariables.VZ_TEAM_ROLE]) ?: return
        val contributorRole = KtBot.getRole(env[EnvVariables.VZ_CONTRIBUTOR_ROLE]) ?: return
        val moderatorRole = KtBot.getRole(env[EnvVariables.VZ_MODERATOR_ROLE]) ?: return

        KtBot.mainGuild!!.findMembersWithRoles(teamRole).onSuccess {
            teamMembers["team"] = it.map { it.author().toModel() }
        }

        KtBot.mainGuild!!.findMembersWithRoles(moderatorRole).onSuccess {
            teamMembers["moderators"] = it
                .filter { !it.roles.contains(teamRole) }
                .map { it.author().toModel() }
        }

        KtBot.mainGuild!!.findMembersWithRoles(contributorRole).onSuccess {
